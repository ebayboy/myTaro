...
#include "ngx_http_waf_header_filter.h"
#include "waf_ip_region_lib.h"
...
static void * waf_load_conf_header_filter(ngx_conf_t *cf, cJSON *root, waf_main_conf_t *waf_conf);
    ...
    const char *first, ...)
{
    ...
    do {
        ...
        if (json_node == NULL) {
            WAF_LOG(NOTICE, pool->log, "Read JSON node [%s] is NULL!", menu);
            va_end(ap_list);
            return NGX_ERROR;
        }
                   ...
                   cJSON *root)
{
    ...
    /* read header_filter check configuration */
    rc = waf_load_conf_header_filter(cf, root, waf_conf);
    if (rc != NGX_OK) {
        WAF_LOG(ERR, cf->log, "func [%s] failed", __func__);
        return NGX_CONF_ERROR;
    }
...
waf_load_big_data_conf(ngx_log_t *log)
{
    ...
    if (fread(data, len, 1, conf_file) != 1) {
        WAF_LOG(ERR, log, "read waf_bd.dat to data failed");
        goto failed;
    }
    if (conf_file) {
        goto failed;
    ...
    if ((root = cJSON_Parse(data)) == NULL) {
        ...
        if (root == NULL) {
            WAF_LOG(ERR, log, "Invalid JSON format in waf_db.dat [%s]", cJSON_GetErrorPtr());
            goto failed;
        }
    }
    ...
    if (rc != NGX_OK) {
        WAF_LOG(ERR, log, "func [%s] failed", __func__);
        goto failed;
    }
    ...
    if (WAF_CONF_BID_DATA_VERSION == big_data->version) {
        ...
        if (rc != NGX_OK) {
            WAF_LOG(ERR, log, "waf_load_conf failed");
            goto failed;
        }
...
void *waf_conf_json_parser(ngx_conf_t *cf, char *data, size_t len, cJSON  **out)
{
	void *rc = NGX_OK;
	cJSON  *root = NULL;
	...
	if ((root = cJSON_Parse(data)) == NULL) {
		/* 将混淆过的配置文件还原为json格式 */
		if (data[0] == '{') {
			WAF_LOG(ERR, cf->log, "Invalid JSON format in waf.dat [%s]", cJSON_GetErrorPtr());
			rc = NGX_CONF_ERROR;
			goto out;
		} else {
			waf_conf_file_decrypt(data, len);
...
/* out : ngx_pfree(cf->pool, data) */
void *waf_read_conf_file(
		ngx_conf_t *cf, 
		ngx_cycle_t *cycle, 
		...
		size_t *olen)
{
	...
	if (waf_check_is_gzip_file((unsigned char *)data, len)) {
		outlen = len << 4;
		gzipout = (unsigned char *) ngx_pcalloc(cf->pool, outlen);
		if (gzipout == NULL) {
            WAF_LOG(ERR,cf->log, "malloc gz buffer failed!");
			rc = NGX_CONF_ERROR;
            goto out;
        }
        if (gzdecompress(data, len, gzipout, &outlen) != 0) {
            WAF_LOG(ERR, cf->log, "gzdecompress waf.dat failed!");
			rc = NGX_CONF_ERROR;
            goto out;
        }
        if (outlen == 0) {
			rc = NGX_CONF_ERROR;
            WAF_LOG(ERR, cf->log, "gzdecompress waf.dat return length:%d!", outlen);
            goto out;
        }
        
        //set gzip buffer
		ngx_pfree(cf->pool, data);
        data = (char *)gzipout;
        len = (int)outlen;

		WAF_LOG(NOTICE, cf->log, "load gzip waf.dat!");
	} else {
		WAF_LOG(NOTICE, cf->log, "load normal waf.dat!");
	}
...
waf_load_conf(ngx_conf_t *cf, ngx_cycle_t *cycle, waf_main_conf_t *waf_conf)
{
	...
	rc = waf_read_conf_file(cf, cycle, waf_conf, &data, &dlen);
	if (rc != NGX_OK) {
		WAF_LOG(ERR, cf->log, "Error:%s:%d", __func__, __LINE__);
		goto out;
	}

	rc = waf_conf_json_parser(cf, data, dlen, &root);
	if (rc != NGX_OK) {
		WAF_LOG(ERR, cf->log, "Error:%s:%d dlen:%i", __func__, __LINE__, dlen);
		goto out;
	}
    ...
    if (rc != NGX_OK) {
        /* 因2.2版没有version字段，暂时注掉，2.3版上线后打开 */
        WAF_LOG(ERR, cf->log, "Error waf_load_version failed!");
        rc = NGX_CONF_ERROR;
        goto out;
    }
    ...
    if (WAF_CONF_VERSION == waf_conf->version) {
        ...
        if (rc != NGX_OK) {
            rc =  NGX_CONF_ERROR;
			WAF_LOG(ERR, cf->log, "Error  waf_load_curr_conf failed!");
            goto out;
        }
...
static ngx_int_t waf_value_std_str_copy(ngx_pool_t *pool, cJSON *cmd, void *data)
{
    u_char *buff;
...
ngx_int_t waf_parse_freq_base_threshold(ngx_pool_t *pool, cJSON *cmd, void *data)
{
    ...
    if (strlen(cmd->valuestring) >= sizeof(reqs) - 1) {
        return NGX_ERROR;
    }
...
ngx_int_t waf_parse_time_base_threshold(ngx_pool_t *pool, cJSON *cmd, void *data)
{
    ...
    ms = (char *)ngx_pcalloc(pool, strlen(cmd->valuestring) + 1);
    if (ms == NULL) {
        return NGX_ERROR;
    }
...
static
ngx_int_t waf_value_str_to_float(ngx_pool_t *pool, cJSON *cmd, void *data)
{
    *(float *)data = atof(cmd->valuestring);
...
waf_load_conf_global(ngx_conf_t *cf, cJSON *root, waf_main_conf_t *waf_conf)
{
    ...
    waf_get_cmd(root, waf_switch_str_to_enum, cf->pool,
                     &waf_conf->cpu_bypass_switch, "CPUByPass", NULL);

    waf_get_cmd(root, waf_value_str_to_float, cf->pool,
            &waf_conf->cpu_bypass_limit, "CPUBypassThreshold", NULL);
    ...
    WAF_LOG(NOTICE, cf->log, "CPUByPass : [%d]", waf_conf->cpu_bypass_switch);
    WAF_LOG(NOTICE, cf->log, "CPUBypassThreshold : [%.2f]", waf_conf->cpu_bypass_limit);
    ...
    if (policy_cnt > 0) {
        ...
        for (i = 0; i < policy_cnt; i++) {
            ...
            if (policy->mode == DLP_MODE_CONTENT) {
                ...
                waf_get_cmd(sub_item, waf_value_str_to_int, cf->pool,
                        &policy->match_id, 
                        "matchid", NULL);
...
static int waf_get_ip_region_lib_buf(unsigned char *output, size_t *outlen)
{
	const unsigned char *bz64 = (const unsigned char *)WAF_IP_REGION_LIB;
	unsigned char *bindata = (unsigned char *)malloc(*outlen);
	int bindata_len = 0;

	//base64 decode
	bindata_len = base64_decode((const char *)bz64, ngx_strlen(bz64), bindata);
	if (bindata_len == 0){
		fprintf(stderr, "Error: %s:%d base64_decode!\n", __func__, __LINE__);
		return -1;
	}

	//gzip decode
	if (!waf_check_is_gzip_file(bindata, bindata_len)) {
		fprintf(stderr, "Error: %s:%d not gzip file!\n", __func__, __LINE__);
		return -1;
	} 

	if (gzdecompress(bindata, bindata_len, output, outlen) != 0) {
		fprintf(stderr, "Error: %s:%d gzdecompress!\n", __func__, __LINE__);
		return -1;
	}
...
static cJSON *waf_find_ip_region_node_from_static(cJSON *root, const char *region_id, ngx_int_t *addr_cnt)
{
	cJSON      *addrs_node = NULL, *rule_node, *rule_sub_node = NULL, *rules_node = NULL;
    int i, rule_cnt;

	if (root == NULL || region_id == NULL || strlen(region_id) == 0) {
		return NULL;
	}

	rule_cnt = waf_get_array_size(root, "IPRegionRule", NULL);
	if (rule_cnt == 0) {
		return NULL;
	}

	// init cfg_ids
	if ((rules_node = waf_get_node(root, "IPRegionRule", NULL)) == NULL) {
		return NULL;
	}
	...
	for (i = 0; i < rule_cnt; i++) {
		...
		/* get cmd id */
		if ((rule_sub_node = waf_get_node(rule_node, "id", NULL)) == NULL) {
			continue;
		}

		if (strncasecmp(region_id, rule_sub_node->valuestring, strlen(region_id)) != 0) {
			continue;
		}

		//return addrs_node
		addrs_node = waf_get_node(rule_node, "addrs", NULL);
		*addr_cnt = waf_get_array_size(rule_node, "addrs", NULL);
		return addrs_node;
	}
...
static cJSON *waf_load_conf_ip_region_static_lib(void)
{
	cJSON      *root = NULL;
	
	unsigned char*out;
	size_t nout = 10*1024*1024;

	if ((out = (unsigned char *)malloc(nout)) == NULL) {
		fprintf(stderr, "Error: %s:%d\n", __func__, __LINE__);
		goto error;
	}

	if (waf_get_ip_region_lib_buf(out, &nout) != 0) {
		fprintf(stderr, "Error: %s:%d\n", __func__, __LINE__);
		goto error;
	}

	//parse json
	if ((root = cJSON_Parse((char *)out)) == NULL) {
		fprintf(stderr, "Error: %s:%d\n", __func__, __LINE__);
		goto error;
	}
...
waf_load_conf_ip_region_rbtree(ngx_conf_t *cf, cJSON *root, waf_main_conf_t *waf_conf)
{
	...
	//malloc CJSON root 
	static_root = waf_load_conf_ip_region_static_lib();
	if (static_root == NULL) {
		WAF_LOG(ERR, cf->log, "Error:waf_load_conf_ip_region_static_lib");
		return NGX_CONF_ERROR;
	}
    ...
    if (rule_cnt > 0) {
        ...
        for (i = 0; i < rule_cnt; i++) {
            ...
            /* get cmd addrs */
            ngx_int_t j, addr_cnt = 0;
            ngx_str_t pstr;
            cJSON *addrs_node = NULL, *addr_node = NULL;
			addr_cnt = waf_get_array_size(rule_node, "addrs", NULL);
			addrs_node = waf_get_node(rule_node, "addrs", NULL);

			//static lib 
			if (addr_cnt == 0 || addrs_node == NULL) {
				WAF_LOG(NOTICE, cf->log, "IPRegion load from static lib id:[%V]", &id);
				addrs_node = waf_find_ip_region_node_from_static(static_root, (const char *)id.data, &addr_cnt);
				if (addrs_node == NULL || addr_cnt == 0) {
					WAF_LOG(ERR, cf->log, "Error %s:%d addrs_node:%p addr_cnt:%i", 
							__func__, __LINE__, addrs_node, addr_cnt);
					continue;
				}
			} else {
				WAF_LOG(NOTICE, cf->log, "IPRegion load from cfg rule->id:[%V] action[%i]" , &id, action);
			}

			WAF_LOG(NOTICE, cf->log, "id:[%V] addr_cnt:%i", &id, addr_cnt);
...
static void *
waf_load_conf_header_filter(ngx_conf_t *cf, cJSON *root, waf_main_conf_t *waf_conf)
{
    ngx_int_t rc;
    cJSON   *sub_root, *req_cookie, *del, *sub_del;
    header_filter_t *hf = &waf_conf->header_filter;
    ngx_memset(hf, 0, sizeof(header_filter_t));

    waf_conf->HeaderFilterEngine = 0;

    sub_root = waf_get_node(root, "HeaderFilter", NULL);
    if (sub_root == NULL) {
        WAF_LOG(NOTICE, cf->log, "no HeaderFilter cmd in waf.dat...skip the function");
        return NGX_OK;
    }

    rc = waf_get_cmd(sub_root,  waf_switch_str_to_enum, cf->pool, 
            &waf_conf->HeaderFilterEngine, "HeaderFilterEngine", NULL);
    if (rc != NGX_OK) {
        WAF_LOG(NOTICE, cf->log, "no HeaderFilterEngine HeaderFilter cmd in waf.dat...skip the function");
        return NGX_CONF_OK;
    }

    WAF_LOG(NOTICE, cf->log, "HeaderFilterEngine:%i", waf_conf->HeaderFilterEngine);

    /* load reqCookie */
    req_cookie = waf_get_node(sub_root, "reqCookie", NULL);
    if (req_cookie == NULL) {
        WAF_LOG(NOTICE, cf->log, "no reqCookiel cmd in waf.dat...skip the function");
        return NGX_OK;
    }

    ngx_int_t j, keys_cnt = waf_get_array_size(req_cookie, "del", NULL);
    if (keys_cnt == 0) {
        WAF_LOG(NOTICE, cf->log, "del array size 0");
        return NGX_OK;
    }
    WAF_LOG(NOTICE, cf->log, "del array size %i", keys_cnt);

    /* get del node from req_cookie */
    if ((del = waf_get_node(req_cookie, "del", NULL)) == NULL) {
        WAF_LOG(NOTICE, cf->log, "no del cmd in waf.dat...skip the function");
        return NGX_OK;
    }

    //header_filter_t;
    hf->reqCookieDel = ngx_array_create(cf->pool, keys_cnt, sizeof(header_filter_cookie_t));
    if ( hf->reqCookieDel == NULL) {
        return NGX_CONF_ERROR;
    }
        
    header_filter_cookie_t  *hfc;

    for(j=0; j< keys_cnt; j++) {
        if ((sub_del  = cJSON_GetArrayItem(del, j)) == NULL) {
            break;
        }
       
        if ((hfc = (header_filter_cookie_t *)ngx_array_push(hf->reqCookieDel)) == NULL) {
            return NGX_CONF_ERROR;
        }
        ngx_memset(hfc, 0, sizeof(*hfc));

        if (!cJSON_IsString(sub_del)) {
            return NGX_CONF_ERROR;
        }

        hfc->key.len = ngx_strlen(sub_del->string);
        hfc->key.data = (u_char *) ngx_pcalloc(cf->pool, ngx_strlen(sub_del->string) + 1);
        ngx_memcpy(hfc->key.data,sub_del->string, hfc->key.len);

        hfc->value.len = ngx_strlen(cJSON_GetStringValue(sub_del));
        hfc->value.data = (u_char *)ngx_pcalloc(cf->pool, hfc->value.len + 1);
        ngx_memcpy(hfc->value.data, cJSON_GetStringValue(sub_del), hfc->value.len);
    }

    ngx_http_waf_header_filter_show(cf->log, waf_conf);
